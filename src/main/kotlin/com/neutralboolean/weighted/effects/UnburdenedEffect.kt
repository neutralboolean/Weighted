...
class UnburdenedEffect: StatusEffect {
    constructor(): super(StatusEffectCategory.BENEFICIAL, 0xffffff)
    ...
    companion object {
        const val UNBURDENED_ID: String = "unburdened"
    }
    ...
    override fun onApplied(entity: LivingEntity?, amplifier: Int) {
        if (entity is StaminaUsingEntity) {
            val id = identifier(UNBURDENED_ID)

            val DEPLETED_DELAY_MULT_MOD: Double = Configs.serverConfig.statusEffectsSettings.unBurdenedEffectStats.get().depletedDelayMult
            val STAMINA_DELAY_MULT_MOD: Double = Configs.serverConfig.statusEffectsSettings.unBurdenedEffectStats.get().staminaDelayMult
            val STAMINA_REGEN_MULT_MOD: Double = Configs.serverConfig.statusEffectsSettings.unBurdenedEffectStats.get().staminaRegenMult
            ...
            if (DEPLETED_DELAY_MULT_MOD != 0.0) {
                ...
                // 1/3 of base depleted stamina regen delay
                val depletedRegenMod = EntityAttributeModifier(id, DEPLETED_DELAY_MULT_MOD, Operation.ADD_MULTIPLIED_BASE)
                depletedStamRegenAttribute!!.updateModifier(depletedRegenMod)
            ...
            if (STAMINA_DELAY_MULT_MOD != 0.0) {
                ...
                // 50% shorter delay in regaining stamina
                val regenDelayMod = EntityAttributeModifier(id, STAMINA_DELAY_MULT_MOD, Operation.ADD_MULTIPLIED_BASE)
                staminaRegenDelayAttribute!!.updateModifier(regenDelayMod)
            ...
            if (STAMINA_REGEN_MULT_MOD != 0.0) {
                ...
                // Double the base stamina regen
                val staminaRegenMod = EntityAttributeModifier(id, STAMINA_REGEN_MULT_MOD, Operation.ADD_MULTIPLIED_BASE)
